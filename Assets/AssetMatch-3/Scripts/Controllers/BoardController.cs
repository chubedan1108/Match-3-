...
public class BoardController : MonoBehaviour
{
    ...
    public event Action OnWinEvent = delegate { };
    public event Action OnLoseEvent = delegate { };

    public bool IsBusy { get; private set; }

    private Board m_board;

    private BottomCell m_bottomCells;
  ...
  //  private bool m_isDragging;
    ...
    private List<Cell> m_potentialMatch; //hint
    ...
    public void StartGame(GameManager gameManager, GameSettings gameSettings)
    {
        ...
        m_bottomCells = new BottomCell(this.transform, gameSettings);
    ...
    {
        //Cell move to new position

        //1. Lay cell con trong trong bottom cell
        Cell target = m_bottomCells.GetEmptyCell();
        if (target == null) return; // full roi --> end game

        //2. Lay item va dang ki vao trong cell moi
        Item item = cell.Item;

        cell.Clear(); //3. Clear cell cu
        ...
        //Check match
        StartCoroutine(CheckBottomMatchCoroutine());
        
    }


    private IEnumerator CheckBottomMatchCoroutine()
    {
        yield return new WaitForSeconds(0.25f);

        var match = m_bottomCells.CheckMatch();
        if (match != null)
        {
            m_bottomCells.RemoveMatch(match);
            m_bottomCells.Compact();
        }
        OnItemAddedToBottom();
    }
    ...
    {
        if (m_bottomCells.IsBottomFull())
        {
            OnLoseEvent();
        }
    }

    private void OnItemAddedToBottom()
    {
        CheckLose();
    }
